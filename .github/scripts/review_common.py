...
def build_prompt(pr_title: str, diff: str, issue_body: str, discussion: str = "", verified_facts: str = "") -> str:
    ...
    return f"""You are a senior engineer reviewing a pull request for **ai-systems-lab**,
a personal portfolio repository of production-quality AI systems engineering projects
(MCP servers, prompt engineering patterns, and reference implementations).

The stack is primarily Python (3.11+), with async I/O, REST/MCP protocol integrations
(GitHub, filesystem, git, email, web search), Docker, and GitHub Actions for CI. Key
constraints: keep individual projects under `projects/` self-contained and independently
runnable, avoid leaking secrets/credentials in example code, and avoid regressions in
CI workflows.{verified_facts_section}
...
Blocking only:
- MCP/API contract or payload shapes misaligned across a change?
- Could this break another project under `projects/`, Docker builds, or CI workflows?
...
For non-blocking improvements (style consistency, missing dependency pin, refactor
