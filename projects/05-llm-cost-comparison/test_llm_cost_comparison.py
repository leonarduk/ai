...
def test_hosted_monthly_cost():
    w = m.Workload(requests_per_day=1000, avg_input_tokens=1_000_000 / 1000, avg_output_tokens=0)
...
def test_hours_needed_for_workload():
    ...
    hours = m.hours_needed_for_workload(total_monthly_tokens=3_600_000, tokens_per_sec=1000)
...
def test_local_monthly_cost_rented():
    assert m.local_monthly_cost_rented(hourly_rate=2.5, hours_needed_per_month=40) == pytest.approx(
        100.0
    )


def test_cost_per_million_tokens():
    assert m.cost_per_million_tokens(monthly_cost=50, monthly_total_tokens=25_000_000) == pytest.approx(
        2.0
    )
...
def test_run_non_interactive_end_to_end(tmp_path: Path, capsys):
    ...
    exit_code = m.run_non_interactive(config_path, export_fmt="json", export_path=export_path)
    ...
    assert {row["option"] for row in exported} == {"Local (owned hardware)", "Claude Opus 5"}
